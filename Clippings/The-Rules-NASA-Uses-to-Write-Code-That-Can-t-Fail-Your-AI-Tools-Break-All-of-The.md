---
title: "The Rules NASA Uses to Write Code That Can’t Fail. Your AI Tools Break All of Them."
author: "David Lee"
site: "Level Up Coding"
published: 2026-03-18T04:09:04Z
source: "https://levelup.gitconnected.com/the-rules-nasa-uses-to-write-code-that-cant-fail-your-ai-tools-break-all-of-them-8d0377998efb"
domain: "levelup.gitconnected.com"
language: "en"
description: "The Rules NASA Uses to Write Code That Can’t Fail. Your AI Tools Break All of Them. In 2006, a NASA engineer wrote ten rules for writing code where lives depended on correctness. They still apply …"
word_count: 1885
---

[Mastodon](https://me.dm/@lordmoma)

## [Level Up Coding](https://levelup.gitconnected.com/?source=post_page---publication_nav-5517fd7b58a6-8d0377998efb---------------------------------------)

Following

![](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*wwiktzUTgtVuGPMNT5aqaw.jpeg)
...
*In 2006, a NASA engineer wrote ten rules for writing code where lives depended on correctness. They still apply. And they’re more urgent now than ever.*

> If you’d like the full deep dive, I expanded it further on my personal blog: [https://leestack.dev/writing/nasa-rules-for-code-that-cant-fail/](https://leestack.dev/writing/nasa-rules-for-code-that-cant-fail/)
...
> *“When it really counts, it may be worth going the extra mile and living within stricter limits than may be desirable.” — Gerard Holzmann, NASA/JPL*

![](https://miro.medium.com/v2/resize:fit:1400/format:webp/1*jXOu6HArf0jQNUwU9zMwmg.png)

[https://www.youtube.com/watch?v=wZpDV-1P9uM](https://www.youtube.com/watch?v=wZpDV-1P9uM)
...
==*AI guardrail: AI loves deeply nested conditionals and early-exit chains. Ask it explicitly to flatten the logic — one level deep where possible. If the generated function has more than two levels of nesting, push back.*==
...
*AI guardrail: AI-generated polling loops, retry logic, and recursive crawlers routinely lack caps. Always ask: what is the maximum number of times this runs, and what happens when we hit it?*
...
==*AI guardrail: AI-generated code opens database connections and file handles without cleanup, especially in error branches. Follow the code through every exit path and confirm it closes what it opened.*==
...
*AI guardrail: AI generates monolithic functions that do many things at once because it’s optimizing for task completion, not maintainability. Set a hard line before you prompt: no function longer than 40–60 lines. Ask for decomposition upfront, not as a refactoring afterthought.*
...
*AI guardrail:* ==*AI-generated code skips validation almost universally*==*. Prompt explicitly: add assertions for expected preconditions before this runs, and postconditions after. Make the assumptions visible and loud.*
...
==*AI guardrail: AI routinely generates empty catch blocks and unchecked return values.*== *Enforce one rule with no exceptions: every error must be logged, raised, or explicitly returned. Nothing gets swallowed. Ever.*
...
*AI guardrail: AI leans on class-level state and module-level globals because they’re easier to generate. Ask it to scope state locally and pass dependencies explicitly — make the data flow visible at every call site.*
...
*AI guardrail: AI buries writes and API calls inside what appear to be utility functions. Ask for a clear structural separation between pure computation and side-effectful operations. The dangerous stuff should be visible, named, and obvious.*

## ABSTRACTION & INDIRECTION

### 09\. One layer of magic.

Abstraction is one of programming’s most powerful tools and one of its most common weapons of self-destruction. Every layer of indirection — every middleware, every dynamic dispatch, every callback chain — makes it harder to answer the question: *what actually runs when I call this?* Holzmann restricted pointer indirection to a single level for exactly this reason. The deeper the chain, the harder the trace.

*AI guardrail: AI stacks abstractions eagerly. After every generation, ask: can this be written more directly? Favour composition you can read linearly over elegance you have to decode. When something breaks at 2am, you want to be able to read the code.*

### 10\. Warnings are errors.

Holzmann required zero compiler warnings from day one — not zero errors, ==zero== ==*warnings*==. The distinction matters. A warning is a future bug that the tools have already found. Treating it as advisory is choosing to leave a known problem unresolved. Type checkers, linters, and static analyzers should run on every change, and their output should be a hard gate, not a soft suggestion.

*AI guardrail: AI-generated code almost never sets up linting or static analysis. These must be part of your project scaffold — wired into CI, configured to fail the build on violations. Set them up before the AI starts writing code, not after.*

## WORKING WITH AI

### 11\. Read every line.

This is the rule that most people working with AI tools are currently violating. AI-generated code is not peer-reviewed code. It has not been tested by someone who cares about whether it works. It has been generated by a system that is very good at producing code that looks correct — and that is precisely the problem. The gap between *looks correct* and *is correct* is where bugs live.
...
*AI guardrail: Never commit code you haven’t read in full. You are still the engineer of record. The AI has no skin in the game.*
...
*AI guardrail: AI optimizes for the happy path.* ==*Tests force it to reason about edge cases and failure modes*== *— which is exactly where AI-generated code tends to be weakest.*
...
> This article is part of my writing at [**leestack.dev**](https://leestack.dev/) — where I go deeper on system design, architecture, and engineering strategy. Interactive labs in progress too. Worth a bookmark.

*Adapted from: Holzmann, G.J. (2006). The Power of Ten — Rules for Developing Safety Critical Code. NASA/JPL Laboratory for Reliable Software.*

<iframe src="https://drive.google.com/viewerng/viewer?url=https%3A//spinroot.com/gerard/pdf/P10.pdf&amp;embedded=true" allowfullscreen="" frameborder="0" height="780" width="600" title=""></iframe>
...
```c
Never forget, AI allows you to produce garbage faster (not sure whether cheaper), but anyway it's still slop, not a real engineering, high quality work.And following the principles from this article allowed us to get to the moon and come back more…
```
...
```c
Didn't they blow an entire mission on a metric-imperial units mismatch? What could justify not using the metric system everywhere?
```
...
```c
Instructions and Data are kept as separate also for security reasons - like sanitizing input to avoid SQL injection or buffer overflows. Great article and thank you for writing!
```
