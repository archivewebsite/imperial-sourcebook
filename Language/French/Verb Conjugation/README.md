# Verb Conjugation

`Verb Conjugation` stores French verb pattern notes that support everyday sentence building.

## What Belongs Here

- high-frequency verbs grouped by useful pattern
- short conjugation reminders for recognition and production
- auxiliary choice notes for common compound forms

## What Does Not

- full tense tables
- grammar notes unrelated to verbs
- vocabulary rows that belong in `../Vocabulary`

## How To Use It

- learn the common verbs first: `être`, `avoir`, `aller`, and `faire`
- group new verbs by pattern instead of memorizing every verb separately
- add example sentences with Indonesian meanings

## Current Notes

- `Core Verb Patterns.md`: starter map for common French verbs and patterns.


## Review Rhythm

- Review when verb patterns, irregular forms, or examples start duplicating grammar notes.
- Keep conjugation tables compact and route usage explanations to grammar notes.

## Sources
- [Lawless French - Regular -er Verbs](https://www.lawlessfrench.com/grammar/regular-er-verbs/)
- [Lawless French - Regular -ir Verbs](https://www.lawlessfrench.com/grammar/regular-ir-verbs/)
- [Lawless French - Regular -re Verbs](https://www.lawlessfrench.com/grammar/regular-re-verbs/)
- [Lawless French - Auxiliary Verbs](https://www.lawlessfrench.com/grammar/auxiliary-verbs/)
