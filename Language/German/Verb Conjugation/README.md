# Verb Conjugation

`Verb Conjugation` stores German verb pattern notes for practical reading and sentence building.

## What Belongs Here

- high-frequency verbs grouped by weak, strong, mixed, modal, auxiliary, separable, and inseparable patterns
- short form reminders for the verbs most likely to appear early
- examples with Indonesian meanings

## What Does Not

- full tense-table collections
- noun gender lists
- general grammar notes unrelated to verbs

## How To Use It

- learn `sein`, `haben`, and `werden` first
- group verbs by pattern before memorizing isolated forms
- mark separable-prefix verbs clearly

## Current Notes

- `Core Verb Patterns.md`: starter German conjugation map.


## Review Rhythm

- Review when verb patterns, irregular forms, or examples start duplicating grammar notes.
- Keep conjugation tables compact and route usage explanations to grammar notes.

## Sources
- [University of Michigan - Strong, Weak and Mixed Verbs](https://resources.german.lsa.umich.edu/grammatik/strong-weak-mixed/)
- [University of Michigan - Modal Verbs](https://resources.german.lsa.umich.edu/grammatik/modal-verbs/)
- [Goethe-Institut - Deutsch Online A1 Glossary](https://lernen.goethe.de/deutschonline/A1/PDF/EN/A1_deutschonline_course_vocabulary_1-18_alphabetical.pdf)
