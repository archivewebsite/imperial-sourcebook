# Tips dan trik matematika

`Tips dan trik matematika` berisi catatan cepat yang dipisah per materi. Setiap file fokus pada cara mengenali pola soal, memilih jalan hitung pendek, dan mengecek hasil.

## Peran folder

- Menyimpan trik yang langsung membantu pengerjaan soal.
- Memisahkan materi berbeda ke file Markdown berbeda agar mudah dicari di Obsidian.
- Menjadi jembatan antara teori umum dan latihan di `Bahas Soal`.

## Indeks utama

Mulai dari [[Tips dan trik matematika MOC]].

## Prinsip isi

- Setiap trik harus punya syarat pakai.
- Rumus ditulis dengan MathJax Obsidian.
- Contoh dibuat pendek agar bisa dipakai sebagai pola ulang.
- Catatan sumber disimpan sebagai tautan biasa, sedangkan hubungan antarcatatan memakai wikilink.

## Rujukan induk

Rujukan utama yang dipakai untuk menyusun kumpulan ini:

- [Khan Academy - Distributive property review](https://www.khanacademy.org/math/arithmetic-home/multiply-divide/properties-of-multiplication/a/distributive-property-review)
- [Khan Academy - Converting between percents, fractions, and decimals](https://www.khanacademy.org/math/cc-sixth-grade-math/x0267d782%3Acc-6th-rates-and-percentages/x0267d782%3Aequivalent-representations-of-percent-problems/a/converting-between-percents-fractions-decimals)
- [OpenStax Algebra and Trigonometry 2e - Quadratic Equations](https://openstax.org/books/algebra-and-trigonometry-2e/pages/2-5-quadratic-equations)
- [OpenStax Algebra and Trigonometry 2e - Logarithmic Properties](https://openstax.org/books/algebra-and-trigonometry-2e/pages/6-5-logarithmic-properties)
- [OpenStax Precalculus 2e - Key Equations, Chapter 7](https://openstax.org/books/precalculus-2e/pages/7-key-equations)
- [OpenStax Precalculus 2e - Key Equations, Chapter 10](https://openstax.org/books/precalculus-2e/pages/10-key-equations)
- [OpenStax Precalculus 2e - Key Equations, Chapter 11](https://openstax.org/books/precalculus-2e/pages/11-key-equations)
- [OpenStax Introductory Statistics 2e - Mathematical Phrases, Symbols, and Formulas](https://openstax.org/books/introductory-statistics-2e/pages/f-mathematical-phrases-symbols-and-formulas)
- [OpenStax Calculus Volume 1 - Key Equations, Chapter 2](https://openstax.org/books/calculus-volume-1/pages/2-key-equations)
- [OpenStax Calculus Volume 1 - Key Equations, Chapter 3](https://openstax.org/books/calculus-volume-1/pages/3-key-equations)
- [OpenStax Calculus Volume 1 - Key Equations, Chapter 5](https://openstax.org/books/calculus-volume-1/pages/5-key-equations)
- [Paul's Online Math Notes - Cheat Sheets](https://tutorial.math.lamar.edu/index.aspx/Extras/mystudents.aspx)
- [Wolfram MathWorld - Number Theory](https://mathworld.wolfram.com/NumberTheory.html)

